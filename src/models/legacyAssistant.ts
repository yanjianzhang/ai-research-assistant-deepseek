...
export class LegacyResearchAssistant {
  ...
  async call(content: string, states: States) {
    try {
      ...
      const { output } = await this.router.call({
        input: content,
        states: simplifyStates(states),
      })
